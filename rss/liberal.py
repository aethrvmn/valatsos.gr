...
CATEGORIES = ['oikonomia', 'apopsi', 'politiki', 'diethni-themata',
              'tehnologia', 'agores']  # Replace with actual category IDs or names
...
def fetch_article_content(article_url):
    ...
    if content_div:
        ...
        # Convert the content div to a string to retain HTML
...
def fetch_and_generate_rss_for_category(category_id):
    ...
    fg.link(
        href=f"https://liberal.gr/katigories/{category_id}", rel="alternate")
    fg.subtitle(f"Arthro apo Liberal.gr Katigorias {category_id}.")
    fg.link(href=f"{HOME_DIR}/liberal.gr/{category_id}.xml", rel="self")
    fg.language("el")

    urls = dict(
        sorted(urls.items(), key=lambda item: item[1]["date"], reverse=True))
